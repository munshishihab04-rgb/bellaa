...
const reviews = [
  ...
  { initial: 'A', email: 'an***@yandex.ru', date: 'March 17, 2026', product: 'Autodesk 3ds Max 2023', productSlug: 'buy-autodesk-3ds-max-2022', text: 'I make 3D models periodically, everything is great in this program, I have been working in it for a long time. Great, I recommend it!', rating: 5 },
  { initial: 'A', email: 'al***@yandex.ru', date: 'March 16, 2026', product: 'Autodesk 3ds Max 2023', productSlug: 'buy-autodesk-3ds-max-2022', text: 'Bought the key for 3ds Max 2023. Fast delivery, within five minutes after payment all the necessary information arrived by email. Activation went without problems. Support responds quickly. The program works stably, all functions are available. Recommend!', rating: 5 },
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            <div className="flex">
              {[1,2,3,4,5]?.map((s) => (
                <svg key={s} width="20" height="20" viewBox="0 0 16 16" fill="#f59e0b">
        ...
        <p className="text-sm text-gray-500 mb-6">
          ...
          We do not verify reviews for authenticity, but we do remove profanity, insults, and advertising.
        </p>
          ...
          {reviews?.map((review, idx) => (
            <div key={idx} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <div className="flex">
                      {[1,2,3,4,5]?.map((s) => (
                        <svg key={s} width="14" height="14" viewBox="0 0 16 16" fill={s <= review?.rating ? '#f59e0b' : '#e5e7eb'}>
                          <path d="M8 1.5l1.8 3.6 4 .6-2.9 2.8.7 4-3.6-1.9-3.6 1.9.7-4L2.2 5.7l4-.6z"/>
                        </svg>
                      ))}
                    </div>
                  ...
                  <Link href={`/product/${review?.productSlug}`} className="text-xs text-[#1c64ff] hover:underline mt-0.5 block">
                    {review?.product}
                  </Link>
                </div>
              </div>
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                <button className="text-xs border border-gray-200 rounded px-2 py-0.5 hover:bg-gray-50 transition-colors">Yes 0</button>
              </div>
